...
public class PageMaker {
    ...
    private void calcData() {
        ...
        endPage = (int) (Math.ceil(cri.getPage() / (double) pageBtnCnt)) * pageBtnCnt;
        startPage = endPage - (pageBtnCnt - 1);

        if (endPage * cri.getRows() < totCnt) {
            next = true;
        } else {
            endPage = (int) (Math.ceil(totCnt / (double) cri.getRows()));
        }
        ...
        this.totPage = (int) (Math.ceil(totCnt / (double) cri.getRows()));
        
        //System.out.println("PageMaker endPage : "+this.endPage);;
    }
