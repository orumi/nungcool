...
//
//		    String UPLOADROOT = config.getServletContext().getRealPath("jsp/web/upload"); // 경로 지정(절대 경로 | ROOT를 기준으로 한 상대경로)
//
...
//
			...
			System.out.println("freq1의값 : " + request.getParameter("freq1"));
			String tag = cd.ReplaceCode(smart.getRequest().getParameter("tag"));
			String schDate = cd.ReplaceCode(smart.getRequest().getParameter("schDate"));
			String mid     = cd.ReplaceCode(smart.getRequest().getParameter("contentId"));	// 지표정의서 ID
			String jugi    = cd.ReplaceCode(smart.getRequest().getParameter("qtr"));		// 주기 
