...
public class GenReqController {
    ...
    public String goProductManageList(ModelMap model, HttpServletRequest request) throws Exception {
        ...
        common.getDetail(genReqService, request, "13");
    ...
    String deleteData(@RequestParam("data") String data) throws Exception {
        ...
        int count = 0;
        try{
            count = genReqService.selectSubPropertyCheck(genReqVO);
        }catch (Exception e){
            System.out.println(e);
            return message = "하위 목록 확인중에 에러가 발생 하였습니다.";
        }
        if(count > 1) {
            message = "하위항목을 먼저 삭제하여 주세요";
            return message;
        }
