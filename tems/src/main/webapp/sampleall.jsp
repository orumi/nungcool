...
function fnOpen() {
	...
	// 필수 - 레포트 파일명
	//oReport.rptname = "Rexpert1";
	//oReport.rptname = "estimate";
	//oReport.rptname = "한글reb파일";
	oReport.rptname = "http://localhost:8080/tems/rebfiles/estimate.reb";
	...
	// 옵션 - 레포트 파라메터
	oReport.param("test").value = "테스트입니다.";
	oReport.param("reqid").value = "20523";
	/* oReport.param("param2").value = "param2";
	oReport.param("param1").value = "param1"; */
	//oReport.paramtype = "index"; // 매개변수 필드를 Index(순서)로 매칭할 경우에 사용.
	...
	*/
	...
	//rex_gsPluginHttpParam = {"test1": "<%=request.getParameter("a")%>", "test2": "<%=request.getParameter("b")%>"};

	// auto refresh time
	//oReport.autorefeshtime = 5;

	// cookie 추가
	//oReport.cookie = document.cookie;
	
	// preservewhitespace 추가 (default : 1)
	//oReport.preservewhitespace = "0";
	
	// oReport.connectiononce = "1"; // 1:true, 0:false(default)
	
	// oReport.httpmethod = "get"; // default(post)
	
	//필수 - 레포트 실행
	//oReport.windowtitle = "Test Title";
	//oReport.iframe(ifrmRexPreview1);
	oReport.open();
	//oReport.open("test"); // 새창의 객체명(target name) 지정
	//oReport.open(800, 600); // 새창의 가로크기, 새창의 새로크기 지정
	//oReport.open("test", 800, 600); // 새창의 객체명(target name), 새창의 가로크기, 새창의 새로크기 지정
...
function fnReportEvent(oRexCtl, sEvent, oArgs) {
	...
	if (sEvent == "init") {
		...
		//oRexCtl.SetCSS("event.printfinishresult.enable=1"); // 이 속성을 1로 설정해야 PrintFinishResult 이벤트가 발생합니다
		//oRexCtl.UpdateCSS();
...
function fnOpenEach() {
	...
	*/
	...
	//oReport.con("ADO1").dataset("ADOSQLDS2").preservewhitespace = "0";
	//oReport.con("ADO1").httpmethod = "get";
	//oReport.con("ADO1").namespace = "ADO1";	// 생략하면, sub id를 사용
	/**/
...
function fnOpenMulti_MainSub() {
	...
	//oReport.reb("reb1").con("XML1").httpmethod = "get";
	...
	//oReport.reb("reb2").con("XML1").preservewhitespace = "0";
...
<body>
	<button onClick="alert(fnRexScriptVersion())">Script Version</button>
	...
	<iframe name="ifrmRexPreview1" id="ifrmRexPreview1" src='rexpreview.jsp' width="100%" height="600"></iframe>
