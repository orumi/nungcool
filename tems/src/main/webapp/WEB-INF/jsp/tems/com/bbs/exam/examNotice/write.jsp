				...
				<div class="col-xs-12">
					...
					<form id="boardForm"  class="form-horizontal">
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input readonly type="text" id="writer" class="form-control" value="<%=nLoginVO.getName()%>" name="writer">
							</div>
							<div class="col-sm-1"></div>
						</div>
...
$("#fileDropZone").on("drop", function(event){
	...
	for(var i = 0, len = files.length; i < len; i++){
		...
		$.ajax({
			url: "<c:url value='/examNotice/attachFile.json' />",
			type: "post",
			data: formData,
			processData:false,
			contentType:false,
...
$('#boardForm').submit(function() {
	...
	var boardVO = {
		...
		modifyID: $("input[name=writer]").val(),
	...
	$.ajax({
		type: 'post',
		url : "<c:url value='/bbs/examNotice/insert.json' />", 
