...
<script>
	  ...
	  //접수내역 메인리스트에서 결재중 팝업 그리드
	    ...
	    gridView.onDataCellClicked = function (grid, index, column) {
			if(index.column == "apprline"){
	    		...
	    		window.location.href="<c:url value='/exam/result/ResultListDetail.do?reqid="+gridView.getValue(index.itemIndex,"reqid")+"'/>";	
	    	}
	...
	function loadData(){
		...
		$.ajax({
		    ...
		    dataType : "json",
		    data : data,
            url: "<c:url value='/exam/result/getRequestList.json'/>",
            success: function (data) {
