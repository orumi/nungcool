...
<div class="col-lg-10">
	<div class="dt-title pull-left">
		...
		<span><%=nLoginVO.getName()%>(<%=nLoginVO.getAdminid()%>) 님이 결재할 문서</span>
	</div>
			...
			<tbody>
				...
				<tr>
					...
					<c:if test="${apprListVO.apprTypeID == 'REFUND'}">
				 		<td class="text-left" ><a href="./apprAgreeReadR.do?apprID=${apprListVO.apprID}&crtPage=${cri.crtPage}&rowCnt=${cri.rowCnt}&searchType=${cri.searchType}&keyword=${cri.keyword}">${apprListVO.apprName}</a></td>
					</c:if>
					<c:if test="${apprListVO.apprTypeID != 'REFUND'}">
				 		<td class="text-left" ><a href="./apprAgreeReadE.do?apprID=${apprListVO.apprID}&crtPage=${cri.crtPage}&rowCnt=${cri.rowCnt}&searchType=${cri.searchType}&keyword=${cri.keyword}">${apprListVO.apprName}</a></td>
					</c:if>
					<td>${apprListVO.name}(${apprListVO.draftID})</td>
					<td>${apprListVO.draftDate}</td>
					<td>${apprListVO.myapprStateName}(${apprListVO.myapprStateID})</td>
		...
		<ul class="pagination">
			...
			<c:if test="${cri.searchType == null&&cri.keyword == null }">
				...
				<c:if test="${pageMaker.prev }">
					<li class="previous">
					<a href="apprAgreeList.do?crtPage=${pageMaker.startPage-1 }&rowCnt=${cri.rowCnt }">Prev</a>
					</li>
				</c:if>
				<c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="idx">
					<li <c:out value="${cri.crtPage==idx? 'class=active' : '' }" /> >
					<a href="apprAgreeList.do?crtPage=${idx }&rowCnt=${cri.rowCnt }">${idx }</a>
					</li>
				</c:forEach>
				<c:if test="${pageMaker.next }">
					<li class="next"">
					<a href="apprAgreeList.do?crtPage=${pageMaker.endPage+1 }&rowCnt=${cri.rowCnt }">Next</a>
					</li>
				</c:if>
			...
			<c:if test="${cri.searchType != null&&cri.keyword != null }">
				...
				<c:if test="${pageMaker.prev }">
					<li class="previous">
					<a href="apprAgreeList.do?crtPage=${pageMaker.startPage-1 }&rowCnt=${cri.rowCnt }&searchType=${cri.searchType }&keyword=${cri.keyword }">Prev</a>
					</li>
				</c:if>
				<c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="idx">
					<li <c:out value="${cri.crtPage==idx? 'class=active' : '' }" /> >
					<a href="apprAgreeList.do?crtPage=${idx }&rowCnt=${cri.rowCnt }&searchType=${cri.searchType }&keyword=${cri.keyword }">${idx }</a>
					</li>
				</c:forEach>
				<c:if test="${pageMaker.next }">
					<li class="next">
