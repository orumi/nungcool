	...
	$(function() {    
		...
		// open Modal Window
		/* addSample  */
	    $( "#btn_addSample" ).button().on( "click", function() {
	...
	function setFrmMember(data){
		...
		if(data["reqid"]){
			...
			$("#rcvtype").prop("checked", true);
			$("#rcvcompany").val(data["cname"]);
			$("#rcvceo").val(data["ceoname"]);
			
			$("#rcvzipcode").val(data["zipcode"]);
			$("#rcvaddr1").val(data["addr1"]);
			$("#rcvaddr2").val(data["addr2"]);
			$("#rcvdept").val(data["mngdept"]);
			$("#rcvmngname").val(data["mngname"]);
			
			$("#rcvemail").val(data["mngemail"]);
			$("#rcvphone").val(data["mngphone"]);
			$("#rcvhp").val(data["mnghp"]);
			$("#rcvfax").val(data["fax"]);
	...
	function setSample(data){
		...
		for(s=0; s<re_sample.length; s++){
			...
			if(reqstate > 1){
				tbl_row += "<td class=\"txt_C b_R_none\">-</td>";
			} else {
				tbl_row += "<td class=\"txt_C b_R_none\"><div class=\"btn_erase\" onclick=\"javascript:actionEvent('eraseSample','"+re_sample[s]["smpid"]+"');\"></div></td>";
			}
			tbl_row += "</tr>";
